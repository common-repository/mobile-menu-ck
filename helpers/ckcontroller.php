...
class CKController {
	...
	public function getModel($name, $base = '\Mobilemenuck\CKModel') {
		if (empty($this->model)) {
			require_once(MOBILEMENUCK_PATH . '/helpers/ckmodel.php');
			require_once(MOBILEMENUCK_PATH . '/models/' . strtolower($name) . '.php');
	...
	public function ajaxAddPicture() {
		require_once MOBILEMENUCK_PATH . '/helpers/ckbrowse.php';
	...
	public function ajaxDoRestoration() {
		...
		$path = MOBILEMENUCK_PATH . '/backup/' . $id . '_bak';
		$content = file_get_contents($path . '/' . $filename);
		$backup = json_decode($content);

		echo str_replace('|CKURIROOT|', MOBILEMENUCK_URI_ROOT, $content);
		exit();
	}
	...
	public function edit() {
		...
		CKFof::redirect(MOBILEMENUCK_ADMIN_EDIT_MENU_URL . '&view=' . $this->view . '&layout=edit&id=' . $editId);
	}
	...
	public function copy() {
		...
		if ($model->copy($id)) {
			Helper::enqueueMessage('Item copied with success');
		} else {
			Helper::enqueueMessage('Error : Item not copied', 'error');
		}

		// Redirect to the edit screen.
//		CKFof::redirect(MOBILEMENUCK_ADMIN_GENERAL_URL);
	}
	...
	public function delete() {
		...
		$model = $this->getModel($this->view);
		if ($model->delete($id)) {
			Helper::enqueueMessage('Item deleted with success');
		} else {
			Helper::enqueueMessage('Error : Item not deleted', 'error');
		}
