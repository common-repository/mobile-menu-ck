...
function ckInitTabsStyles() {
	$ck('div.cktab:not(.current)').hide();
	$ck('.cktablink').each(function(i, tab) {
		$ck(tab).click(function() {
			$ck('div.cktab[data-group="'+$ck(tab).attr('data-group')+'"]').hide();
			$ck('.cktablink[data-group="'+$ck(tab).attr('data-group')+'"]').removeClass('current');
...
function ckPreviewStylesparams() {
	...
	var myurl = MOBILEMENUCK_ADMIN_EDIT_STYLE_URL + "&task=style.ajaxRenderCss&" + CKTOKEN;
	$ck.ajax({
		type: "POST",
		url: myurl,
		data: {
		...
		alert(CK.Text._('CK_FAILED', 'Failed'));
	});
}
...
function ckSaveEdition(button) {
	if (! $ck('#name').val()) {
		...
		alert(CK.Text._('CK_PLEASE_GIVE_NAME', 'Please give a name'));
		return;
	}
	// if ($ck('#initialname').val() !== '' && $ck('#initialname').val() !== $ck('#name').val()) {
		// var createnewstyle = confirm(CK.Text._('CK_CONFIRM_SAVE_AND_COPY'));
		// if (createnewstyle) {
			// $ck('#id').val('0');
		// }
	// }
	...
	customstyles = JSON.stringify(customstyles);
	var myurl = MOBILEMENUCK_ADMIN_EDIT_STYLE_URL + "&task=style.ajaxSaveStyles&" + CKTOKEN;
...
function ckLoadPreset(name) {
	...
	var myurl = MOBILEMENUCK_ADMIN_EDIT_STYLE_URL + "&task=style.ajaxLoadPresetFields&" + CKTOKEN;
	$ck.ajax({
		type: "POST",
		url: myurl,
		dataType: 'json',
...
function ckLoadPresetCustomcss(name) {
	...
	var myurl = MOBILEMENUCK_ADMIN_EDIT_STYLE_URL + "&task=style.ajaxLoadPresetCustomcss&" + CKTOKEN;
	$ck.ajax({
		type: "POST",
		url: myurl,
		data: {
